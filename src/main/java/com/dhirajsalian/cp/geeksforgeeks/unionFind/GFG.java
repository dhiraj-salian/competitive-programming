...
package com.dhirajsalian.cp.geeksforgeeks.unionFind;
...
class Solution {
    ...
    public void union_(int a, int b, int par[], int rank[]) {
        int aRep = find(a, par), bRep = find(b, par);
        if (aRep == bRep) return;
        if (rank[aRep] < rank[bRep]) par[aRep] = bRep;
        else if (rank[bRep] < rank[aRep]) par[bRep] = aRep;
        else {
            par[bRep] = aRep;
            rank[aRep]++;
        }
